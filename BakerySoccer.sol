...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    /**
     * @dev See {IERC721Enumerable-getAllToken}.
     */
    function getAllToken() public view  returns (AskEntry[] memory) {
        // _tokenOwners are indexed by tokenIds, so .length() returns the number of tokenIds
        AskEntry[] memory allToken = new AskEntry[](_tokenOwners.length());
        for (uint256 i = 0; i < _tokenOwners.length(); ++i) {
            (uint256 tokenId, address to) = _tokenOwners.at(i);
            allToken[i] = AskEntry({tokenId: tokenId, to: to});
        }
        return allToken;
    }
...
// File: contracts/soccer/BakerySoccer.sol
...
contract BakerySoccer is ERC721Pausable, AccessControl, Ownable {
    uint256 public nextTokenId = 1;
    event Burn(address indexed sender, uint256 tokenId);
    
    

    constructor() public ERC721('AFX NFT', 'AfricaSwap NFT miner') {
